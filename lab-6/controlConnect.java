...
class modelConnect extends Observable{
	...
	*/
	 ...
	 */
	public int humanScoreCalc(){																		//Change this functionnnnnnnnn
	 ...
	 * @return					Returns the Score	
	 */
	public int compScoreCalc(){																			//Change this functionnnnnnnnnnnnnnn
		int counter1=0,counter2=0;
	...
	*/
	...
	public boolean checkIfPiecesCanBeDroppedIn(int column){
		...
		if(column<0||column>24){
			setChanged();
			notifyObservers("Invalid column");
			clearChanged();
			return false;
		}
		for(;position<8;){
		...
		if(position!=-1){
			...
			setChanged();
			notifyObservers("Column is full! Please choose something else");
			clearChanged();
			return false;
		}
	}
        ...
        do {
                for ( int index = 0; index < 2; index ++ )      {
                        ...
                        if ( isItaDraw() )      {
                                ...
                                thePlayers.elementAt(index).nextMove();
...
public class controlConnect {
	...
	public static void main( String[] args ) {
		viewConnect aView=new viewConnect();
		modelConnect Model=aView.aModel;
		//controlConnect aControl=new controlConnect();
		Player aPlayer = new Player(Model, "A", '+');
		//public Player bPlayer = new Player(aControl.aModel, "B", '*');
		Computer bPlayer = new Computer(Model, "wIsh", '$');
		Model.addObserver(aView);
		aPlayer.addObserver(aView);
...
class viewConnect implements Observer{
	static Scanner sc=new Scanner(System.in);
	modelConnect aModel=new modelConnect();
	void input(){
		System.out.println("Please enter the column where you want to play");
		int position=sc.nextInt();
		aModel.setColumn(position);
	}
